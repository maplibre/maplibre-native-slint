...
namespace {
class SlintMetalRenderableResource final
    : public mbgl::mtl::RenderableResource {
public:
    SlintMetalRenderableResource(MetalRendererBackend& backend_,
                                 CA::MetalLayer* layer_)
        : backend(backend_), layer(layer_),
          commandQueue(
              NS::TransferPtr(backend.getDevice()->newCommandQueue())) {
        assert(layer);
    ...
    void bind() override {
        ...
        // Note: NS::RetainPtr already handles retain/release, no need for
        // additional CFRetain
        MTL::Texture* externalTex = surface->texture();
        backend.setExternalDrawable(externalTex);

        CGSize ds = layer->drawableSize();
        mbgl::Size texSize{static_cast<uint32_t>(ds.width),
                           static_cast<uint32_t>(ds.height)};
        commandBuffer = NS::RetainPtr(commandQueue->commandBuffer());
        ...
        renderPassDescriptor->colorAttachments()->object(0)->setClearColor(
            MTL::ClearColor{0.95, 0.95, 0.95, 1.0});  // Light gray
        ...
        if (buffersInvalid || !depthTexture || !stencilTexture) {
            ...
            depthTexture->setFormat(mbgl::gfx::TexturePixelType::Depth,
            ...
            static_cast<Texture2D*>(depthTexture.get())
                ->setUsage(MTL::TextureUsageShaderRead |
    ...
    void swap() override {
        ...
        surface.reset();
        // Clear the cached drawable texture to ensure fresh drawable on next
        // frame
...
MetalRendererBackend::MetalRendererBackend(CA::MetalLayer* layer)
    : mbgl::mtl::RendererBackend(mbgl::gfx::ContextMode::Unique),
      mbgl::gfx::Renderable(
          mbgl::Size{0, 0},
          std::make_unique<SlintMetalRenderableResource>(*this, layer)) {
}
