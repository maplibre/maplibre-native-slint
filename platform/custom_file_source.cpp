    ...
    void request(const Resource& resource, Callback callback) {
        auto future = std::async(std::launch::async, [url = resource.url]() {
            ...
            if (r.error.code != cpr::ErrorCode::OK) {
                ...
                response.error = std::make_unique<Response::Error>(
                    Response::Error::Reason::Server, "HTTP status code " + std::to_string(r.status_code));
            } else {
...
std::unique_ptr<AsyncRequest> CustomFileSource::request(const Resource& resource, Callback callback) {
    impl->request(resource, std::move(callback));
    // A proper implementation would return a request object that can be cancelled.
