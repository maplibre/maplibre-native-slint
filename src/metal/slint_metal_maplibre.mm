...
namespace {
// Minimal RendererFrontend that directly owns mbgl::Renderer and updates it
// synchronously.
class SlintMetalRendererFrontend : public mbgl::RendererFrontend {
public:
    SlintMetalRendererFrontend(std::unique_ptr<mbgl::Renderer> renderer_,
    ...
    static const mbgl::TaggedScheduler& tagged() {
        static mbgl::TaggedScheduler sched{mbgl::Scheduler::GetBackground(),
    ...
    void renderNow() {
        if (renderer && pendingParameters) {
            ...
            const mbgl::gfx::BackendScope scope(
    ...
    bool hasParameters() const {
        return pendingParameters != nullptr;
    }
    mbgl::Renderer* get() {
        return renderer.get();
    }
...
struct SlintMetalMapLibre::Impl : public mbgl::MapObserver,
                                  public mbgl::RendererObserver {
    ...
    Impl() {
        ...
        clientOptions.withName("SlintMetalMapLibre").withVersion("1.0.0");
    ...
    void ensureMap(CA::MetalLayer* layer, int w, int h, float pixelRatio) {
        if (!backend) {
            backend =
                std::make_unique<slint_metal::MetalRendererBackend>(layer);
        }
        ...
        if (!frontend) {
            auto renderer = std::make_unique<mbgl::Renderer>(
                *backend, pixelRatio, std::nullopt);
            frontend = std::make_unique<SlintMetalRendererFrontend>(
                std::move(renderer), *backend);
        ...
        if (!map) {
            ...
            cam.withZoom(
                5.0);  // Increased from 2.0 for better initial tile loading
            map->jumpTo(cam);
        } else {
    ...
    void onDidFinishRenderingFrame(
        const mbgl::MapObserver::RenderFrameStatus& status) override {
        // Only log significant events to reduce spam
        static int frameCount = 0;
        static int continuousRepaints = 0;

        if (++frameCount % 60 == 0 || status.placementChanged) {
            std::cout << "[MapObserver] Frame " << frameCount
                      << " - needsRepaint: " << status.needsRepaint
                      << ", placementChanged: " << status.placementChanged
                      << std::endl;
        }
        ...
        if (status.needsRepaint) {
            continuousRepaints++;
            // Only request more frames if we haven't been continuously
            // repainting forever
    ...
    void onDidFailLoadingMap(mbgl::MapLoadError error,
        ...
        std::cout << "[MapObserver] Failed to load map error "
                  << static_cast<int>(error) << ": " << message << std::endl;
        // Try a different style
        if (owner && owner->m_impl && owner->m_impl->map) {
            std::cout << "[MapObserver] Trying fallback style" << std::endl;
            owner->m_impl->map->getStyle().loadURL(
                "https://demotiles.maplibre.org/style.json");
        }
    }
    void onDidFailLoadingTile(const std::string& tileId,
                              const std::string& error) {
        std::cout << "[MapObserver] Failed to load tile " << tileId << ": "
                  << error << std::endl;
        // Don't spam the console with tile loading errors
        static int tileErrorCount = 0;
        if (++tileErrorCount <= 5) {
            std::cout << "[MapObserver] Tile loading errors detected - check "
    ...
    void onResourceError(std::exception_ptr error) override {
        try {
            ...
            std::cerr << "[RendererObserver] Resource error: " << e.what()
                      << std::endl;
        }
    }
    ...
    void onWillStartRenderingFrame() override {
        ...
        static int frameCount = 0;
        if (++frameCount % 60 == 0) {
            std::cout << "[RendererObserver] Will start rendering frame "
                      << frameCount << std::endl;
        }
    }

    void onDidFinishRenderingFrame(
        mbgl::RendererObserver::RenderMode mode, bool repaint,
        ...
        if (++frameCount % 60 == 0) {
            std::cout << "[RendererObserver] Finished rendering frame "
                      << frameCount << " (mode: "
                      << (mode == mbgl::RendererObserver::RenderMode::Full
                              ? "Full"
                              : "Partial")
                      << ", repaint: " << repaint
        ...
        if (repaint) {
            ...
            continuousRepaints =
                0;  // Reset counter when not continuously repainting
        }
    }
...
void SlintMetalMapLibre::initialize(int logicalWidth, int logicalHeight,
                                    float devicePixelRatio) {
    std::cout << "[SlintMetalMapLibre] initialize(" << logicalWidth << ","
              << logicalHeight << ", dpr=" << devicePixelRatio << ")"
              << std::endl;
...
void SlintMetalMapLibre::ensureLayer() {
    ...
    @autoreleasepool {
        ...
        if (!dev) {
            std::cerr << "[SlintMetalMapLibre] Failed to create Metal device!"
                      << std::endl;
            return;
        }
        ...
        std::cout << "[SlintMetalMapLibre] Created CAMetalLayer with device: "
                  << dev << " contentsScale: " << m_dpr << std::endl;

        m_layerPtr = static_cast<void*>(layer);
    }
}

void SlintMetalMapLibre::resize(int logicalWidth, int logicalHeight,
                                float devicePixelRatio) {
    std::cout << "[SlintMetalMapLibre] resize(" << logicalWidth << ","
              << logicalHeight << ", dpr=" << devicePixelRatio << ")"
              << std::endl;
    m_logicalWidth = logicalWidth;
    m_logicalHeight = logicalHeight;
    ...
    @autoreleasepool {
        ...
        std::cout << "[SlintMetalMapLibre] Updated layer contentsScale="
                  << m_dpr << " drawableSize=" << drawableSize.width << "x"
                  << drawableSize.height << std::endl;
    }
}
...
void SlintMetalMapLibre::setStyleUrl(const std::string& url) {
    ...
    if (m_impl && m_impl->map) {
        std::cout << "[SlintMetalMapLibre] Loading style immediately"
                  << std::endl;
        try {
            m_impl->map->getStyle().loadURL(url);
            m_styleApplied = true;
            std::cout
                << "[SlintMetalMapLibre] Style loadURL called successfully"
                << std::endl;
        } catch (const std::exception& e) {
            std::cerr << "[SlintMetalMapLibre] Exception loading style: "
                      << e.what() << std::endl;
        }
    } else {
        std::cout << "[SlintMetalMapLibre] Style URL set for later loading"
                  << std::endl;
    }
}
...
bool SlintMetalMapLibre::render_once() {
    if (!m_layerPtr) {
        std::cout
            << "[SlintMetalMapLibre] render_once() - No layer pointer, skipping"
            << std::endl;
        return false;
    }
    ...
    if (testMode) {
        @autoreleasepool {
            ...
            renderPass->colorAttachments()->object(0)->setTexture(
                drawable->texture());
            renderPass->colorAttachments()->object(0)->setLoadAction(
                MTL::LoadActionClear);
            renderPass->colorAttachments()->object(0)->setStoreAction(
                MTL::StoreActionStore);
            renderPass->colorAttachments()->object(0)->setClearColor(
                MTL::ClearColor{1.0, 0.0, 1.0, 1.0});  // Magenta
    ...
    @autoreleasepool {
        CA::MetalLayer* layer = static_cast<CA::MetalLayer*>(m_layerPtr);
        if (!m_impl) {
            std::cout << "[SlintMetalMapLibre] render_once() - Creating impl"
                      << std::endl;
        ...
        if (!m_impl->map) {
            std::cout << "[SlintMetalMapLibre] render_once() - No map, skipping"
                      << std::endl;
            return false;
        }
        if (!m_styleApplied && !m_pendingStyleUrl.empty()) {
            std::cout << "[SlintMetalMapLibre] render_once() - Loading style: "
                      << m_pendingStyleUrl << std::endl;
            m_impl->map->getStyle().loadURL(m_pendingStyleUrl);
            m_styleApplied = true;
        }
        std::cout << "[SlintMetalMapLibre] render_once() -> processing frame"
                  << std::endl;
        try {
            ...
            if (m_impl->frontend->hasParameters()) {
                std::cout
                    << "[SlintMetalMapLibre] Rendering frame with parameters"
                    << std::endl;
                m_impl->frontend->renderNow();
            } else {
                std::cout
                    << "[SlintMetalMapLibre] No update parameters available yet"
                    << std::endl;
            }
        } catch (const std::exception& e) {
            std::cerr << "[SlintMetalMapLibre] render_once() - Exception: "
                      << e.what() << std::endl;
            return false;
        }
    }
...
void SlintMetalMapLibre::handle_mouse_press(float x, float y) {
    std::cout << "[Interaction] Mouse press at (" << x << ", " << y << ")"
              << " (previous position was " << m_last_x << ", " << m_last_y
              << ")" << std::endl;
...
void SlintMetalMapLibre::handle_mouse_release(float x, float y) {
    std::cout << "[Interaction] Mouse release at (" << x << ", " << y << ")"
              << std::endl;
...
void SlintMetalMapLibre::handle_mouse_move(float x, float y, bool pressed) {
    if (!m_impl || !m_impl->map) {
        std::cout << "[Interaction] No map available for mouse move"
                  << std::endl;
        return;
    }
    ...
    if (!pressed && m_pressed) {
        std::cout << "[Interaction] Implicit mouse release detected"
                  << std::endl;
    ...
    if (pressed && m_pressed) {
        ...
        // This is because Slint calls mouse_press after threshold, then
        // immediately mouse_move
        if (m_move_count == 1) {
            std::cout << "[Interaction] First move after press, resetting "
                         "position to ("
                      << x << ", " << y << ")" << std::endl;
            m_last_x = x;
            m_last_y = y;
        ...
        if (std::abs(dx) > 0.1 || std::abs(dy) > 0.1) {
            std::cout << "[Interaction] Mouse move by (" << dx << ", " << dy
                      << ")" << std::endl;

            // Use BackendScope for Metal context management during map
            // operations
            try {
                const mbgl::gfx::BackendScope scope(
                    *m_impl->backend,
                ...
                std::cout << "[Interaction] Map moved successfully"
                          << std::endl;
                ...
                std::cerr << "[Interaction] Exception during map move: "
                          << e.what() << std::endl;
                return;
            }
            ...
            if (m_request_frame) {
                std::cout << "[Interaction] Requesting frame update"
                          << std::endl;
                m_request_frame();
            } else {
                std::cout << "[Interaction] No frame request callback available"
                          << std::endl;
            }
        } else {
...
void SlintMetalMapLibre::handle_double_click(float x, float y, bool shift) {
    if (!m_impl || !m_impl->map)
        return;
    auto cam = m_impl->map->getCameraOptions();
    double currentZoom = cam.zoom.value_or(0.0);
    double delta = shift ? -1.0 : 1.0;
    double targetZoom =
        std::min(m_max_zoom, std::max(m_min_zoom, currentZoom + delta));
    mbgl::CameraOptions next;
    next.withCenter(m_impl->map->latLngForPixel(mbgl::ScreenCoordinate{x, y}));
    next.withZoom(targetZoom);
    m_impl->map->jumpTo(next);
...
void SlintMetalMapLibre::handle_wheel_zoom(float x, float y, float dy) {
    ...
    // Invert the zoom direction: scroll up (dy > 0) zooms in, scroll down (dy <
    // 0) zooms out
...
void SlintMetalMapLibre::fly_to(const std::string& location) {
    ...
    animationOptions.duration =
        std::chrono::milliseconds(2000);  // 2 second animation
