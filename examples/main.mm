...
struct MetalFrameSchedulerCFRunLoop {
    static std::atomic<bool>& needFrame() {
    ...
    static std::weak_ptr<slint_metal::SlintMetalMapLibre>& weakMap() {
        static std::weak_ptr<slint_metal::SlintMetalMapLibre> w;
        return w;
    }
    static CFRunLoopTimerRef& timer() {
        static CFRunLoopTimerRef t = nullptr;
        return t;
    }
    static std::atomic<uint64_t>& lastFrameNS() {
        static std::atomic<uint64_t> v{0};
        return v;
    }
    static uint64_t targetIntervalNS() {
        return 16'666'666ull;
    }  // 60Hz

    static void setup(
        const std::shared_ptr<slint_metal::SlintMetalMapLibre>& map) {
        weakMap() = map;
        if (timer() == nullptr) {
            ...
            timer() = CFRunLoopTimerCreate(
                ...
                [](CFRunLoopTimerRef, void*) {
                    ...
                    if (++renderCount % 60 == 0) {
                        std::cout << "[FrameScheduler] Frame " << renderCount
                                  << std::endl;
                    }

                    // Clear the needFrame flag before rendering to prevent
                    // duplicate renders
                    ...
                    // Check if another frame is needed immediately (for smooth
                    // interaction)
                    if (needFrame().load(std::memory_order_relaxed)) {
    ...
    static void request(
        ...
        bool expected = false;
        if (needFrame().compare_exchange_strong(expected, true,
                                                std::memory_order_relaxed)) {
            // Only schedule if we successfully set the flag (prevents duplicate
            // scheduling)
            setup(map);
        }
    }
...
int main(int argc, char** argv) {
    std::cout << "[main] Starting application (Metal layer embedding)"
              << std::endl;
    ...
    // Frame requests come from MapLibre invalidate callbacks; no periodic tick
    // needed Connect backend frame request to scheduler
    slint_map_libre->set_frame_request_callback(
        [weak = std::weak_ptr<slint_metal::SlintMetalMapLibre>(
             slint_map_libre)]() {
            if (auto sp = weak.lock()) {
                MetalFrameSchedulerCFRunLoop::request(sp);
            }
        });
    ...
    main_window->global<MapAdapter>().on_fly_to(
        [=](const slint::SharedString& loc) {
            slint_map_libre->fly_to(std::string(loc.data(), loc.size()));
        });
    ...
    main_window->on_map_size_changed([=]() {
        ...
        if (w > 0 && h > 0) {
            if (!*initialized) {
                ...
                std::cout << "[main] Window found: " << (window ? "yes" : "no")
                          << std::endl;
                std::cout << "[main] MapLibre valid: "
                          << (slint_map_libre->valid() ? "yes" : "no")
                          << std::endl;
                if (window && slint_map_libre->valid()) {
                    @autoreleasepool {
                        CAMetalLayer* metalLayer = static_cast<CAMetalLayer*>(
                            slint_map_libre->metalLayer());
                        std::cout << "[main] Metal layer: "
                                  << (metalLayer ? "yes" : "no") << std::endl;
                        if (metalLayer) {
                            std::cout << "[main] Creating Metal view..."
                                      << std::endl;
                            std::cout << "[main] Window content bounds: "
                                      << window.contentView.bounds.size.width
                                      << "x"
                                      << window.contentView.bounds.size.height
                                      << std::endl;
                            std::cout << "[main] Map size from Slint: " << w
                                      << "x" << h << std::endl;
                            std::cout
                                << "[main] ContentView isFlipped: "
                                << (window.contentView.isFlipped ? "YES" : "NO")
                                << std::endl;

                            // Calculate the frame for the map area only (not
                            // the entire window) Slint lays out: controls at
                            // top (~82px), map below (518px of 600px total) The
                            // black area at bottom in the screenshot is where
                            // Slint's map Rectangle is We need to position our
                            // Metal view to match that location

                            CGFloat windowHeight =
                                window.contentView.bounds.size.height;
                            CGFloat controlsHeight =
                                windowHeight -
                                h;  // Controls take up the difference
                            ...
                            if (window.contentView.isFlipped) {
                                // If flipped, y=0 is at top, so map should
                                // start after controls
                                mapFrame.origin.y =
                                    controlsHeight;  // Start after controls
                            } else {
                                // If not flipped, y=0 is at bottom (standard
                                // macOS)
                            ...
                            std::cout << "[main] Positioning map: y="
                                      << mapFrame.origin.y
                            ...
                            NSView* metalView =
                                [[NSView alloc] initWithFrame:mapFrame];
                            ...
                            std::cout << "[main] Existing subviews count: "
                                      << subviews.count << std::endl;

                            // Find if there's a Slint view we should position
                            // relative to
                            NSView* slintView = nil;
                            if (subviews.count > 0) {
                                slintView = subviews[0];  // Slint's view should
                                                          // be the first one
                                std::cout
                                    << "[main] Found existing view frame: "
                                    << slintView.frame.origin.x << ","
                                    << slintView.frame.origin.y
                                    << " size: " << slintView.frame.size.width
                                    << "x" << slintView.frame.size.height
                                    << std::endl;
                            }
                            ...
                            // We need to insert our Metal view at the correct
                            // position The black area at bottom is Slint's
                            // transparent Rectangle where map should be
                            ...
                            std::cout
                                << "[main] Configuring layer properties..."
                                << std::endl;
                            ...
                            std::cout
                                << "[main] Attached CAMetalLayer size=" << w
                                << "x" << h << " dpr=" << dpr << std::endl;
                        }
                    }
                } else {
                    std::cout << "[main] Failed to attach CAMetalLayer - "
                                 "window or layer not available"
                              << std::endl;
                }

                std::cout << "[main] Initialized maplibre size=" << w << "x"
                          << h << std::endl;
                std::cout << "[main] Expect warmup frames while tiles load"
                          << std::endl;
                *initialized = true;
            } else {
                ...
                if (window && window.contentView.subviews.count > 0) {
                    @autoreleasepool {
                        // Find the Metal view (should be the last subview we
                        // added)
                        ...
                        if (metalView) {
                            ...
                            // Check if the coordinate system is flipped (it
                            // should be)
                            if (window.contentView.isFlipped) {
                                // If flipped, y=0 is at top, so map should
                                // start after controls
                            ...
                            mapFrame.size.height =
                                h;  // Use the actual map height from Slint
                ...
                std::cout << "[main] Resized maplibre surface to " << w << "x"
                          << h << std::endl;
    ...
    // (Not strictly required since process exiting, but keeps tools quiet if
    // reused) NOTE: Accessing internal scheduler static state replicated here
    // would require exposing it; skipped.
    return 0;
}
