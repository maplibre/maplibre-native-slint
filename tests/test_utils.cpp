...
void OpenGLTest::SetUp() {
    // Note: In a real test environment, you would need to initialize an OpenGL context
    // For headless testing, consider using EGL or a similar solution
}
...
void MockFileSource::setResponse(const std::string& url, const std::string& data) {
    responses[url] = data;
}
...
bool compareImages(const std::vector<uint8_t>& img1, 
    ...
    for (size_t i = 0; i < pixelCount; ++i) {
        ...
        double rDiff = std::abs(static_cast<int>(img1[baseIndex]) - static_cast<int>(img2[baseIndex]));
        double gDiff = std::abs(static_cast<int>(img1[baseIndex + 1]) - static_cast<int>(img2[baseIndex + 1]));
        double bDiff = std::abs(static_cast<int>(img1[baseIndex + 2]) - static_cast<int>(img2[baseIndex + 2]));
        double aDiff = std::abs(static_cast<int>(img1[baseIndex + 3]) - static_cast<int>(img2[baseIndex + 3]));
...
std::vector<uint8_t> createTestTexture(int width, int height, uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
