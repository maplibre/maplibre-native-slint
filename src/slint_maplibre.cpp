...
slint::Image SlintMapLibre::render_map() {
  ...
  for (int i = 0;
    ...
    std::cout << "(" << (int)raw_data[offset] << ","
              << (int)raw_data[offset + 1] << "," << (int)raw_data[offset + 2]
  ...
  for (int i = 0; i < unpremult_image.size.width * unpremult_image.size.height;
       i++) {
...
void SlintMapLibre::handle_mouse_press(float x, float y) { last_pos = {x, y}; }
