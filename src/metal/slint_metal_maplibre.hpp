...
public:
    ...
    // Initialize (creates CAMetalLayer if not already). logical sizes are in
    // logical (DPI independent) units.
    void initialize(int logicalWidth, int logicalHeight,
    ...
    // Provide a callback that will be invoked when the map backend requests
    // another frame.
    void set_frame_request_callback(std::function<void()> cb) {
        m_request_frame = std::move(cb);
    }

    // Trigger one render (stub for now). Returns true if a frame was issued.
    bool render_once();

    // Expose underlying CAMetalLayer pointer (as void*) for embedding in native
    // view hierarchy.
    void* layerPtr() const {
        return m_layerPtr;
    }

    // Raw pointer to underlying CAMetalLayer (opaque outside Objective-C++
    // translation unit).
...
private:
    ...
    void* m_layerPtr =
        nullptr;  // CA::MetalLayer* (retained) - created/owned here
    ...
    struct Impl;
    std::unique_ptr<Impl>
        m_impl;  // will hold mbgl::Map, backend, renderer, etc.
