...
TEST_F(MapRenderingTest, RenderWithMouseInteraction) {
    ...
    // Both renders should succeed
    EXPECT_TRUE(true);  // In a real test, you might compare images to ensure
                        // they're different
}
...
TEST_F(MapRenderingTest, StressTestRapidRenders) {
    ...
    // Log performance info (in a real test, you might have performance
    // assertions)
    std::cout << "Rendered " << num_renders << " frames in " << duration.count()
              << "ms" << std::endl;
}